module.exports = async function(req, res) {
  ...
  try {
    ...
    const a = await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:500,messages:[{role:'user',content:[{type:'image',source:{type:'url',url}},{type:'text',text:'Extract receipt data. Respond ONLY with JSON: {"amount":<number>,"category":"<Food|Transport|Office Supplies|Software|Marketing|Equipment|Utilities|Other>","note":"<vendor>","date":"<YYYY-MM-DD or null>"}'}]}]})});
