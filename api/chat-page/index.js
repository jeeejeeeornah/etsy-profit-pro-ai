...
async function loadUserData() {
  ...
  try {
    ...
    if (data && data.length > 0) {
      const d = data[0];
      systemPrompt = 'Du bist ein hilfreicher KI-Steuerberater. Finanzdaten des Nutzers: Einnahmen: €' + (d.total_income||0).toFixed(2) + ', Ausgaben: €' + (d.total_expenses||0).toFixed(2) + ', Gewinn: €' + (d.profit||0).toFixed(2) + '. Antworte auf Deutsch, kurz und klar.';
    }
  } catch(e) {}
}
